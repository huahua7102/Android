...
public class DownloadTask extends AsyncTask<String,Integer,Integer> {
    ...
    public DownloadTask(DownloadListener listener){
        this.listener = listener;//将下载的状态通过listener进行回调
    }
    ...
    protected Integer doInBackground(String... params) {
        ...
        try{
            ...
            if(file.exists()){
                downloadedLength = file.length();//已下载的文件大小
            }
            ...
            if(contentLength == 0){
                return  TYPE_FAILED;
            }else if(contentLength == downloadedLength){//已存在
                return  TYPE_SUCCESS;
            }
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        if(progress > lastProgress){
            listener.onProgress(progress);//在UI更新进度
